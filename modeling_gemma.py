...
class KVCache:
    ...
    def num_items(self) -> int:
        if len(self.key_cache) == 0:
            ...
            # Shape of the key_cache is [Batch_size, Nb_head_KV, Seq_len, Head_Dim]
...
class GemmaRMSNorm:
    ...
    def __init__(self, dim: int, eps: float) -> None :
        ...
        self.eps = eps
        self.dim = dim
        self.weights = nn.Parameter(torch.zeros(self.dim))

    def _norm(self, x):
        return x / torch.sqrt(torch.pow(x, 2).mean(-1, keepdim=True) + self.eps) 

    def forward(self, x):
        output = self._norm(x)
        output = output * self.weights.float()
...
class GemmaRotaryEmbedding:
    def __init__(self, dim: int, max_position_embedding: int= 2048, base: int= 10000) -> None:
        super().__init__()
        self.dim = dim
        self.max_position_embedding = max_position_embedding
        self.base = base

        inv_freq = 1.0 / (self.base ** (torch.arrange(0, self.dim, 2, dtype= torch.int64).float() / self.dim))
...
class GemmaAttention(nn.Module):
    def __init__(self, config: GemmaConfig, layer_idx: Optional[int]) -> None :
        ...
        self.Wq = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias)
        self.Wk = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        self.Wv = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
...
class GemmaDecoderLayer(nn.Module):
    def __init__(self, config: GemmaConfig, layer_idx: int) -> None :
        ...
        self.self_attn = GemmaAttention(config= self.config, layer_idx= layer_idx)

        self.MLP = GemmaMLP(self.config)

        self.input_LN = GemmaRMSNorm(self.hidden_size, self.config.rms_norm_eps)
        self.post_attn_LN = GemmaRMSNorm(self.hidden_size, self.config.rms_norm_eps)
...
class GemmaModel(nn.Module):
    def __init__(self, config: GemmaConfig) -> None:
        ...
        self.RMSnorm = GemmaRMSNorm(config.hidden_size, eps= config.rms_norm_eps)
...
class PaliGemmaForConditionalGeneration:
    def __init__(self, config: PaliGemmaConfig) -> None:
        ...
        self.multi_modal_proj = PaliGemmaMultiModalProjector(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(self, input_ids: torch.LongTensor = None, 
        ...
        image_features = self.multi_modal_proj(selected_image_feature)
